...
- 中文 / English 双语界面
- 项目标题、任务表格、负责人、开始日期、结束日期、工作日自动计算
- 可选展示字段：`阶段`、`状态`
- 内置 AR、3D、数字化项目排期模板
- 支持按阶段快捷添加任务
...
3. 选择是否展示 `阶段` 和 `状态` 字段。
4. 使用内置模板，或点击 `+ 新增任务` / 阶段快捷按钮添加任务。
5. 为每行填写任务、负责人、开始日期和结束日期。
6. 打开 `排期预览` 检查甘特图和表格。
7. 点击 `导出 Excel` 下载排期文件。
...
- 支持中文或英文表头，跟随网页语言导出
- 开启 `阶段` 后会导出为 `Model` / `工作内容` 列，并按阶段合并单元格
- 开启 `状态` 后会导出状态列
- 日期轴按中国工作日生成
...
点击 `导入排期`，选择由本工具导出的 `.xlsx` 文件。系统会尝试读取：
